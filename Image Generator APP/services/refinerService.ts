// Agent 4: Master Refiner (Client-Side Canvas Implementation)
...
export const REFINER_PRESETS: Record<string, RefinerPreset> = {
  cinematic: {
    ...
    brightness: 1.02,
    vignette: true,
    filmGrain: false, // Removed film grain
  ...
  photorealistic: {
    ...
    sharpen: 1.1, // Reduced
    contrast: 1.08,
    saturation: 1.0, // Reduced, rely on vibrance
    brightness: 1.0,
    vignette: true,
    filmGrain: false, // Removed film grain entirely
    colorGrade: 'none',
    microContrast: 1.05, // Subtle microcontrast
    clarityBoost: 0.1, // Reduced
    shadowLift: 5,
    highlightRecovery: -3,
    vibrance: 1.08,
    chromaticAberration: 0.2, // Drastically reduced
    lensDistortion: 0.01, // Drastically reduced
    shadowTint: { r: 250, g: 252, b: 255 }, // Subtle cool shadows
    highlightTint: { r: 255, g: 253, b: 250 }, // Subtle warm highlights
  },
  artistic: {
    name: "Artistic Boost",
...
/**
 * Agent 4: Master Refiner (Client-Side Canvas Version)
 * Applies professional post-processing to a base64 encoded image.
 */
export async function refineImage(
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      // --- Put data back to canvas ONCE ---
      ctx.putImageData(imageData, 0, 0);

      // --- All Canvas context operations ---
...
// --- EXISTING & ENHANCED FILTER IMPLEMENTATIONS ---
...
function applyMicroContrast(imageData: ImageData, amount: number): ImageData {
  ...
  // Unsharp mask is a better way to implement micro-contrast
...
function applyClarity(imageData: ImageData, amount: number): ImageData {
  ...
  const height = imageData.height;
  const output = new Uint8ClampedArray(data);
  const blurData = applyFastGaussian(new Uint8ClampedArray(data), width, height, 5); // Larger radius blur
  for (let i = 0; i < data.length; i += 4) {
    const r = data[i], g = data[i+1], b = data[i+2];
    const lum = 0.299 * r + 0.587 * g + 0.114 * b;
    const blurLum = 0.299 * blurData[i] + 0.587 * blurData[i+1] + 0.114 * blurData[i+2];
    if (lum > 50 && lum < 200) { // Apply to mid-tones
...
function applyColorTints(
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    if (lum < 80) {
      const strength = (80 - lum) / 80 * 0.1; // Reduced strength
    ...
    if (lum > 200) {
      const strength = (lum - 200) / 55 * 0.05; // Reduced strength
...
function applyColorGrade(ctx: CanvasRenderingContext2D, width: number, height: number, grade: string) {
  ...
  switch (grade) {
    case 'teal-orange':
      ...
      ctx.globalAlpha = 0.1; // Reduced opacity
    ...
    case 'natural':
      ...
      ctx.fillRect(0, 0, width, height);
      break;
    case 'vibrant': break; // Handled by saturation
  }
...
function applyVignette(ctx: CanvasRenderingContext2D, width: number, height: number) {
  ...
  gradient.addColorStop(1, 'rgba(0,0,0,0.15)'); // Reduced opacity
...
function applyFilmGrain(ctx: CanvasRenderingContext2D, width: number, height: number) {
  // This function is now effectively disabled by setting filmGrain to false in presets.
  // Kept here for potential future use with a better algorithm.
  const imageData = ctx.getImageData(0, 0, width, height);
  const data = imageData.data;
  for (let i = 0; i < data.length; i += 4) {
    const noise = (Math.random() - 0.5) * 10; // Very subtle noise amount
    data[i] += noise;
    data[i + 1] += noise;
    data[i + 2] += noise;
  }
...
// --- UTILITY ---
// Fast Gaussian blur for use in other filters
...
// FIX: Add missing implementations for boxBlurH and boxBlurT to complete the fast Gaussian blur utility.
