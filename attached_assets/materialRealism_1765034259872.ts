import { BrandStyleKey, MaterialPreset, MaterialPresetKey, PlatformKey } from "../../types";

// ============================================================================
// FABRIC PHYSICS (CONSOLIDATED)
// ============================================================================

export interface FabricPhysics {
  fabricType: string;
  weight: string;          // oz/yd²
  stretchability: number;  // 0-100
  drapeFactor: number;     // How fabric hangs, 0-100
  wrinkleTendency: number; // 0-100
...
export const FABRIC_PHYSICS: Record<string, FabricPhysics> = {
  'cotton-ringspun': {
    ...
    weight: '4.2-4.5 oz/yd²',
    stretchability: 15,
    drapeFactor: 40,
    wrinkleTendency: 65,
    textureDensity: 'Medium weave with visible cotton fibers',
...
export function getFabricPhysics(fabricDescription: string): FabricPhysics | null {
  const normalized = fabricDescription.toLowerCase();
  ...
  // Default to ring-spun cotton if unknown
  return FABRIC_PHYSICS['cotton-ringspun'];
}


// ============================================================================
// PRINT METHOD PHYSICS
// ============================================================================
...
const PRINT_METHODS_MAP: Record<string, PrintMethod> = {
    dtg: DTG_PRINTING,
    'screen-print': SCREEN_PRINTING,
    sublimation: SUBLIMATION_PRINTING,
};

export function getPrintMethodGuidance(productType: string): PrintMethod {
  if (productType.includes('aop') || productType.includes('sublimation')) {
    return PRINT_METHODS_MAP.sublimation;
  }
  return PRINT_METHODS_MAP.dtg; // default
}
...
export const MATERIAL_PRESETS: Record<MaterialPresetKey, MaterialPreset> = {
  ...
  BRAND_NEW: {
    ...
    colorFade: 0,
    pilling: 'none',
    fabricTexture: 'crisp unworn texture with slight stiffness',
    ...
    bestFor: ['E-commerce main images', 'Product catalogs', 'New product launches', 'Professional listings'],
    platformFit: ['Amazon', 'Shopify', 'Etsy (new items)', 'Print Materials']
  },
  ...
  LIVED_IN: {
    ...
    colorFade: 5,
    pilling: 'minimal',
    fabricTexture: 'natural cotton weave with slight micro-pilling',
    ...
    bestFor: ['Lifestyle photography', 'Authentic looks', 'Social media content', 'Realistic mockups'],
    platformFit: ['Instagram', 'Social Media', 'Lifestyle Blogs', 'Etsy']
  },

  WELL_WORN: {
    id: 'WELL_WORN',
    name: 'Well Worn',
    description: 'Frequently worn and washed, soft comfortable fabric, visible character',
    age: 12,
    washCycles: 25,
    foldIntensity: 9,
    microwrinkles: true,
    ...
    colorFade: 15,
    pilling: 'light',
    fabricTexture: 'soft well-worn cotton with visible texture and character',
    promptAddition: 'Well-worn favorite garment washed approximately 25 times over 12 months, highly natural fabric folds (9+ prominent creases and extensive micro-wrinkles), light pilling visible particularly on high-friction areas, colors faded by approximately 15% creating vintage appearance, fabric is very soft with maximum drape and flow, authentic favorite t-shirt quality, significant character marks from regular use, comfortable broken-in texture',
    bestFor: ['Vintage aesthetic', 'Street photography', 'Authentic lifestyle', 'Worn-in look'],
    platformFit: ['Instagram', 'Pinterest', 'Vintage stores', 'Street fashion']
  },
  ...
  VINTAGE_DISTRESSED: {
    ...
    colorFade: 30,
    pilling: 'moderate',
    fabricTexture: 'heavily textured vintage fabric with authentic aging',
    ...
    bestFor: ['Vintage brands', 'Retro aesthetic', 'Distressed looks', 'Nostalgic marketing'],
    platformFit: ['Etsy Vintage', 'Depop', 'Vintage Blogs', 'Retro brands']
  },

  PREMIUM_MAINTAINED: {
    id: 'PREMIUM_MAINTAINED',
    name: 'Premium Maintained',
    description: 'High-quality garment with careful maintenance, elegant appearance',
    age: 6,
    washCycles: 10,
    foldIntensity: 5,
    microwrinkles: false,
    ...
    colorFade: 2,
    pilling: 'none',
    fabricTexture: 'premium fabric with maintained quality and refined appearance',
    promptAddition: 'Premium quality garment with careful maintenance, washed approximately 10 times with proper care, controlled natural folds (5-6 elegant creases maintaining shape), no pilling due to high-quality fabric and proper care, colors remain vibrant with only 2% fade showing premium quality, fabric softness is refined and luxurious, elegant maintained drape, sophisticated well-cared-for appearance, premium textile quality evident in texture, professional garment care visible',
    bestFor: ['Premium brands', 'Luxury products', 'High-end fashion', 'Professional photography'],
    platformFit: ['Luxury e-commerce', 'Premium Shopify', 'High-end catalogs', 'Brand websites']
  }
};

export function recommendMaterialPreset(platform: PlatformKey, brandStyle: BrandStyleKey): MaterialPresetKey {
    if (platform === 'Amazon' || brandStyle === 'ECOMMERCE_CLEAN') {
        return 'BRAND_NEW';
    }
    if (brandStyle === 'VINTAGE_RETRO') {
        return 'VINTAGE_DISTRESSED';
    }
    if (brandStyle === 'EDITORIAL_FASHION') {
        return 'PREMIUM_MAINTAINED';
    }
    if (brandStyle === 'STREET_URBAN') {
        return 'WELL_WORN';
    }
