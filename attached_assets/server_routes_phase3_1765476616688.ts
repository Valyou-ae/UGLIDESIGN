...
export async function registerRoutes(
  ...
  const getSafeErrorMessage = (error: unknown, fallback: string = "An error occurred"): string => {
    ...
    for (const pattern of sensitivePatterns) {
      if (pattern.test(message)) {
        logger.debug("Sanitized error (sensitive pattern detected)", { message, source: "security" });
        return fallback;
      }
    }
  ...
  const requireAdmin = async (req: any, res: any, next: any) => {
    requireAuth(req, res, async () => {
      try {
        ...
        logger.error("Admin auth error", error, { source: "api" });
  ...
  app.get("/api/auth/user", requireAuth, async (req: any, res) => {
    try {
      ...
      logger.error("Error fetching user", error, { source: "api" });
  ...
  app.post("/api/auth/google", authRateLimiter, async (req: any, res) => {
    try {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
          try {
            ...
            if (isDnsError && attempt < maxRetries) {
              ...
              logger.warn(`DNS error on attempt ${attempt}/${maxRetries}, retrying in ${delay}ms`, { source: "database" });
      ...
      logger.error("Google auth error", error, { source: "api" });
  ...
  app.post("/api/auth/forgot-password", passwordResetLimiter, async (req, res) => {
    try {
      ...
      if (user) {
        ...
        // For now, log the reset link (only visible in development)
        ...
        logger.debug(`Password reset link generated for ${normalizedEmail}`, { resetLink, source: "auth" });
      }
      ...
      logger.error("Forgot password error", error, { source: "api" });
  ...
  app.post("/api/auth/reset-password", async (req, res) => {
    try {
      ...
      logger.error("Reset password error", error, { source: "api" });
  ...
  app.get("/api/stripe/config", async (_req, res) => {
    try {
      ...
      logger.error("Stripe config error", error, { source: "api" });
  ...
  app.get("/api/stripe/products", requireAuth, async (_req, res) => {
    try {
      ...
      logger.error("Stripe products error", error, { source: "api" });
  ...
  app.post("/api/stripe/create-checkout-session", requireAuth, async (req: any, res) => {
    try {
      ...
      logger.error("Stripe checkout error", error, { source: "api" });
  ...
  app.post("/api/stripe/create-portal-session", requireAuth, async (req: any, res) => {
    try {
      ...
      logger.error("Stripe portal error", error, { source: "api" });
  ...
  app.get("/api/stripe/subscription-status", requireAuth, async (req: any, res) => {
    try {
      ...
      logger.error("Stripe subscription status error", error, { source: "api" });
  ...
  app.get("/api/images/calendar", requireAuth, async (req: any, res) => {
    try {
      ...
      logger.error("Calendar data error", error, { source: "api" });
      res.status(500).json({ message: "Server error" });
    }
  });
  ...
  app.get("/api/images/by-date", requireAuth, async (req: any, res) => {
    try {
      ...
      logger.error("Images by date error", error, { source: "api" });
      res.status(500).json({ message: "Server error" });
    }
  });
  ...
  app.get("/api/gallery", async (req: any, res) => {
    try {
      ...
      logger.error("Gallery error", error, { source: "api" });
      res.status(500).json({ message: "Server error" });
    }
  });
  ...
  app.post("/api/gallery/:imageId/like", requireAuth, async (req: any, res) => {
    try {
      ...
      logger.error("Like error", error, { source: "api" });
      res.status(500).json({ message: "Server error" });
    }
  });
  ...
  app.post("/api/guest/generate-image", guestGenerationLimiter, async (req, res) => {
    try {
      ...
      logger.error("Guest generation error", error, { source: "api" });
      return res.status(500).json({ message: "Generation failed" });
    }
  });
  ...
  app.post("/api/generate/analyze", requireAuth, generationRateLimiter, async (req, res) => {
    try {
      ...
      logger.error("Analysis error", error, { source: "api" });
  ...
  app.post("/api/generate/draft", requireAuth, generationRateLimiter, async (req, res) => {
    try {
      ...
      logger.error("Draft generation error", error, { source: "api" });
    ...
    try {
      ...
      if (result) {
        ...
        try {
          ...
          logger.error("Failed to save image to database", saveError as Error, { source: "generation" });
          sendEvent("final_image", {
            index: 0,
            imageData: result.imageData,
            mimeType: result.mimeType,
      ...
      logger.error("Final generation error", error, { source: "api" });
      res.write(`event: error\ndata: ${JSON.stringify({ message: "Generation failed" })}\n\n`);
      res.end();
    }
  });
  ...
  app.post("/api/generate/single", requireAuth, generationRateLimiter, async (req: any, res) => {
    try {
      ...
      logger.error("Single generation error", error, { source: "api" });
  ...
  app.post("/api/mockup/analyze", requireAuth, async (req, res) => {
    try {
      ...
      logger.error("Design analysis error", error, { source: "api" });
      res.status(500).json({ message: "Analysis failed" });
    }
  });
  ...
  app.post("/api/mockup/generate", requireAuth, generationRateLimiter, async (req: any, res) => {
    try {
      ...
      logger.error("Mockup generation error", error, { source: "api" });
  ...
  app.post("/api/mockup/generate-batch", requireAuth, generationRateLimiter, async (req: any, res) => {
    try {
      ...
      if (modelDetails) {
        ...
        logger.debug("Elite mockup generation started", {
          source: "mockup",
        ...
        try {
          for (let sizeIndex = 0; sizeIndex < sizesToGenerate.length; sizeIndex++) {
            ...
            if (sizeIndex === 0 && batch.personaLock) {
              ...
              logger.debug("Persona lock captured - will reuse for subsequent sizes", { source: "mockup" });
            }
          ...
          logger.error("Elite mockup batch error", batchErr as Error, { source: "mockup" });
      ...
      logger.error("Batch mockup generation error", error, { source: "api" });
  ...
  app.get("/api/elite-mockup/products", requireAuth, async (_req, res) => {
    try {
      ...
      logger.error("Products fetch error", error, { source: "api" });
  ...
  app.get("/api/elite-mockup/brand-styles", requireAuth, async (_req, res) => {
    try {
      ...
      logger.error("Brand styles fetch error", error, { source: "api" });
  ...
  app.post("/api/elite-mockup/analyze", requireAuth, async (req, res) => {
    try {
      ...
      logger.error("Elite design analysis error", error, { source: "api" });
      res.status(500).json({ message: "Analysis failed" });
    }
  });
  ...
  app.post("/api/elite-mockup/generate", requireAuth, async (req, res) => {
    try {
      ...
      logger.error("Elite mockup generation error", error, { source: "api" });
  ...
  app.post("/api/elite-mockup/refine", requireAuth, async (req, res) => {
    try {
      ...
      logger.error("Elite mockup refinement error", error, { source: "api" });
      res.status(500).json({ message: "Refinement failed" });
    }
  });
  ...
  app.post("/api/seamless-pattern/ai-enhanced", requireAuth, async (req, res) => {
    try {
      ...
      logger.error("AI seamless pattern generation error", error, { source: "api" });
      res.status(500).json({ 
        success: false, 
  ...
  app.post("/api/background-removal", requireAuth, generationRateLimiter, async (req: any, res) => {
    try {
      ...
      logger.error("Background removal error", error, { source: "api" });
      res.status(500).json({ 
        success: false, 
  ...
  app.post("/api/background-removal/batch", requireAuth, generationRateLimiter, async (req, res) => {
    try {
      ...
      try {
        ...
        logger.error("Batch background removal error", batchError as Error, { source: "background-removal" });
      ...
      logger.error("Background removal batch error", error, { source: "api" });
  ...
  app.get("/api/background-removal/presets", requireAuth, async (_req, res) => {
    try {
      ...
      logger.error("Background removal presets error", error, { source: "api" });
      res.status(500).json({ 
        success: false, 
  ...
  app.get("/api/admin/users", requireAdmin, async (req: any, res) => {
    try {
      ...
      logger.error("Admin users fetch error", error, { source: "api" });
  ...
  app.patch("/api/admin/users/:id/role", requireAdmin, async (req: any, res) => {
    try {
      ...
      logger.error("Admin role update error", error, { source: "api" });
  ...
  app.get("/api/admin/crm/contacts", requireAdmin, async (_req: any, res) => {
    try {
      ...
      logger.error("Admin contacts fetch error", error, { source: "api" });
  ...
  app.get("/api/admin/crm/contacts/:id", requireAdmin, async (req: any, res) => {
    try {
      ...
      logger.error("Admin contact fetch error", error, { source: "api" });
  ...
  app.post("/api/admin/crm/contacts", requireAdmin, async (req: any, res) => {
    try {
      ...
      logger.error("Admin contact create error", error, { source: "api" });
  ...
  app.patch("/api/admin/crm/contacts/:id", requireAdmin, async (req: any, res) => {
    try {
      ...
      logger.error("Admin contact update error", error, { source: "api" });
  ...
  app.delete("/api/admin/crm/contacts/:id", requireAdmin, async (req: any, res) => {
    try {
      ...
      logger.error("Admin contact delete error", error, { source: "api" });
  ...
  app.get("/api/admin/crm/deals", requireAdmin, async (_req: any, res) => {
    try {
      ...
      logger.error("Admin deals fetch error", error, { source: "api" });
  ...
  app.get("/api/admin/crm/deals/:id", requireAdmin, async (req: any, res) => {
    try {
      ...
      logger.error("Admin deal fetch error", error, { source: "api" });
  ...
  app.post("/api/admin/crm/deals", requireAdmin, async (req: any, res) => {
    try {
      ...
      logger.error("Admin deal create error", error, { source: "api" });
  ...
  app.patch("/api/admin/crm/deals/:id", requireAdmin, async (req: any, res) => {
    try {
      ...
      logger.error("Admin deal update error", error, { source: "api" });
  ...
  app.delete("/api/admin/crm/deals/:id", requireAdmin, async (req: any, res) => {
    try {
      ...
      logger.error("Admin deal delete error", error, { source: "api" });
  ...
  app.get("/api/admin/crm/activities", requireAdmin, async (_req: any, res) => {
    try {
      ...
      logger.error("Admin activities fetch error", error, { source: "api" });
  ...
  app.post("/api/admin/crm/activities", requireAdmin, async (req: any, res) => {
    try {
      ...
      logger.error("Admin activity create error", error, { source: "api" });
  ...
  app.patch("/api/admin/crm/activities/:id", requireAdmin, async (req: any, res) => {
    try {
      ...
      logger.error("Admin activity update error", error, { source: "api" });
  ...
  app.delete("/api/admin/crm/activities/:id", requireAdmin, async (req: any, res) => {
    try {
      ...
      logger.error("Admin activity delete error", error, { source: "api" });
  ...
  app.get("/api/admin/analytics", requireAdmin, async (_req: any, res) => {
    try {
      ...
      logger.error("Admin analytics fetch error", error, { source: "api" });
  ...
  app.post("/api/prompts/favorites", requireAuth, async (req: any, res) => {
    try {
      ...
      logger.error("Create prompt favorite error", error, { source: "api" });
  ...
  app.get("/api/prompts/favorites", requireAuth, async (req: any, res) => {
    try {
      ...
      logger.error("Get prompt favorites error", error, { source: "api" });
  ...
  app.delete("/api/prompts/favorites/:id", requireAuth, async (req: any, res) => {
    try {
      ...
      logger.error("Delete prompt favorite error", error, { source: "api" });
  ...
  app.get("/api/mood-boards", requireAuth, async (req: any, res) => {
    try {
      ...
      logger.error("Get mood boards error", error, { source: "api" });
  ...
  app.post("/api/mood-boards", requireAuth, async (req: any, res) => {
    try {
      ...
      logger.error("Create mood board error", error, { source: "api" });
  ...
  app.get("/api/mood-boards/:id", requireAuth, async (req: any, res) => {
    try {
      ...
      logger.error("Get mood board error", error, { source: "api" });
  ...
  app.patch("/api/mood-boards/:id", requireAuth, async (req: any, res) => {
    try {
      ...
      logger.error("Update mood board error", error, { source: "api" });
  ...
  app.delete("/api/mood-boards/:id", requireAuth, async (req: any, res) => {
    try {
      ...
      logger.error("Delete mood board error", error, { source: "api" });
  ...
  app.post("/api/mood-boards/:id/items", requireAuth, async (req: any, res) => {
    try {
      ...
      logger.error("Add item to board error", error, { source: "api" });
  ...
  app.patch("/api/mood-boards/:boardId/items/:itemId", requireAuth, async (req: any, res) => {
    try {
      ...
      logger.error("Update board item error", error, { source: "api" });
  ...
  app.delete("/api/mood-boards/:boardId/items/:itemId", requireAuth, async (req: any, res) => {
    try {
      ...
      logger.error("Remove item from board error", error, { source: "api" });
