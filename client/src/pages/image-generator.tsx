                ...
                >
                  ...
                  <div className="mt-3">
                    <button 
                      ...
                      className="w-full flex items-center justify-between px-3 py-2 bg-gradient-to-r from-purple-500/10 to-pink-500/10 hover:from-purple-500/20 hover:to-pink-500/20 border border-purple-500/20 rounded-lg transition-all group"
                    >
                      <div className="flex items-center gap-2">
                        <div className="h-6 w-6 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 flex items-center justify-center">
                          <Users className="h-3.5 w-3.5 text-white" />
                        </div>
                        <span className="text-xs font-semibold text-foreground">Advanced AI Studio</span>
                        <Badge variant="secondary" className="text-[9px] bg-purple-500/20 text-purple-400 border-purple-500/30">
                          5-Agent System
                        </Badge>
                      </div>
                      <ChevronDown className={cn("h-4 w-4 text-muted-foreground transition-transform", showAdvancedPanel && "rotate-180")} />
                    </button>
                    ...
                    <AnimatePresence>
                      {showAdvancedPanel && (
                        <motion.div
                          ...
                          className="overflow-hidden"
                        >
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3 p-3 bg-background/50 border border-border/50 rounded-lg">
                            
                            {/* 5-Agent System Toggle */}
                            <div className="space-y-2">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                  <Users className="h-4 w-4 text-purple-500" />
                                  <label className="text-[11px] font-semibold text-foreground">5-Agent System</label>
                                </div>
                                <Switch 
                                  checked={settings.useMultiAgent}
                                  onCheckedChange={(c) => setSettings({...settings, useMultiAgent: c})}
                                  className="scale-90 data-[state=checked]:bg-purple-500"
                                  data-testid="switch-multi-agent"
                                />
                              </div>
                              <p className="text-[10px] text-muted-foreground leading-relaxed">
                                Director, Cinematographer, Lighting, Color & Refiner agents collaborate for 50-60% quality boost
                              </p>
                            </div>
                            
                            {/* Draft to Final Toggle */}
                            <div className="space-y-2">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                  <Film className="h-4 w-4 text-pink-500" />
                                  <label className="text-[11px] font-semibold text-foreground">Draft to Final</label>
                                </div>
                                <Switch 
                                  checked={settings.useDraftToFinal}
                                  onCheckedChange={(c) => setSettings({...settings, useDraftToFinal: c})}
                                  className="scale-90 data-[state=checked]:bg-pink-500"
                                  data-testid="switch-draft-to-final"
                                />
                              </div>
                              <p className="text-[10px] text-muted-foreground leading-relaxed">
                                Generate draft first, then apply Cinematic DNA polish for Hollywood-quality output
                              </p>
                            </div>
                            ...
                            <div className="space-y-2">
                              <div className="flex items-center gap-2">
                                ...
                                <label className="text-[11px] font-semibold text-foreground">Artistic Style</label>
                                <Badge variant="outline" className="text-[9px] ml-auto">
                                  {artisticStyles.length} styles
                                </Badge>
                              </div>
                              <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                  <Button 
                                    variant="outline" 
                                    className="w-full justify-between h-9 text-xs bg-background border-border hover:bg-muted"
                                    data-testid="dropdown-artistic-style"
                                  >
                                    <span className="truncate">
                                      {settings.artisticStyle === "auto" 
                                        ? "Auto Detect" 
                                        : artisticStyles.find(s => s.id === settings.artisticStyle)?.name || settings.artisticStyle
                                      }
                                    </span>
                                ...
                                <DropdownMenuContent align="end" className="w-[220px] max-h-[320px] overflow-y-auto">
                                  <DropdownMenuItem 
                                    onClick={() => setSettings({...settings, artisticStyle: "auto"})}
                                  ...
                                  {Array.from(new Set(artisticStyles.map(s => s.category))).map(category => (
                                    <div key={category}>
                                      ...
                                      {artisticStyles
                                        ...
                                        .map(style => (
                                          <DropdownMenuItem 
                                            key={style.id}
                                            onClick={() => setSettings({...settings, artisticStyle: style.id})}
                                          ...
                                          >
                                            <div className="flex items-center gap-2 w-full">
                                              <div 
                                                ...
                                                style={{ 
                                                  background: style.colorPalette?.[0] || '#888'
                                                }}
                                              />
                                              <span className="truncate">{style.name}</span>
                                              {settings.artisticStyle === style.id && (
                            ...
                            <div className="mt-3 p-3 bg-purple-500/5 border border-purple-500/20 rounded-lg">
                              <div className="flex items-start gap-3">
                                <div className="flex -space-x-2">
                                  <div className="h-7 w-7 rounded-full bg-blue-500 flex items-center justify-center border-2 border-background">
                                    <Crown className="h-3.5 w-3.5 text-white" />
                                  </div>
                                  <div className="h-7 w-7 rounded-full bg-purple-500 flex items-center justify-center border-2 border-background">
                                    <Camera className="h-3.5 w-3.5 text-white" />
                                  </div>
                                  <div className="h-7 w-7 rounded-full bg-amber-500 flex items-center justify-center border-2 border-background">
                                    <Lightbulb className="h-3.5 w-3.5 text-white" />
                                  </div>
                                  <div className="h-7 w-7 rounded-full bg-pink-500 flex items-center justify-center border-2 border-background">
                                    <Palette className="h-3.5 w-3.5 text-white" />
                                  </div>
                                  <div className="h-7 w-7 rounded-full bg-green-500 flex items-center justify-center border-2 border-background">
                                    <Sparkles className="h-3.5 w-3.5 text-white" />
                                  </div>
                                </div>
                                <div className="flex-1">
                                  <p className="text-[11px] font-medium text-foreground">Cinematic DNA Enhancement Active</p>
                                  <p className="text-[10px] text-muted-foreground mt-0.5">
                                    Volumetric Atmosphere, Professional Lighting, Depth Layering, Color Grading, Material Rendering, Cinematic Composition, Cinema Camera Systems
                                  </p>
                                </div>
                              </div>
                            </div>
